<?php

namespace App\Validators;


use App\Table\PostTable;

/**
 * Objet charger de validé les articles
 *
 * Class PostValidator
 * @package App\Validators
 */
final class PostValidator extends AbstractValidator
{

    public function __construct(array $data, PostTable $postTable, array $categoriesIds, ?int $postID = null)
    {
        parent::__construct($data);
        $this->validator->rule("required", ["name", "slug"]);
        $this->validator->rule("lengthBetween", ["name", "slug"], 3, 200);
        $this->validator->rule("slug", "slug");
        $this->validator->rule("subset", "categories_ids", array_keys($categoriesIds));
        $this->validator->rule(function ($field, $value) use ($postTable, $postID) {
            return !$postTable->exist($field, $value, $postID);
        },"slug", "Cette valeur est déjà utilisée");
    }

}
